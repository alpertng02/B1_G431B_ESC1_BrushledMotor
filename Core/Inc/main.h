/* USER CODE BEGIN Header */
/**
  ******************************************************************************
  * @file           : main.h
  * @brief          : Header for main.c file.
  *                   This file contains the common defines of the application.
  ******************************************************************************
  * @attention
  *
  * Copyright (c) 2026 STMicroelectronics.
  * All rights reserved.
  *
  * This software is licensed under terms that can be found in the LICENSE file
  * in the root directory of this software component.
  * If no LICENSE file comes with this software, it is provided AS-IS.
  *
  ******************************************************************************
  */
/* USER CODE END Header */

/* Define to prevent recursive inclusion -------------------------------------*/
#ifndef __MAIN_H
#define __MAIN_H

#ifdef __cplusplus
extern "C" {
#endif

/* Includes ------------------------------------------------------------------*/
#include "stm32g4xx_hal.h"

/* Private includes ----------------------------------------------------------*/
/* USER CODE BEGIN Includes */

/* USER CODE END Includes */

/* Exported types ------------------------------------------------------------*/
/* USER CODE BEGIN ET */

/* USER CODE END ET */

/* Exported constants --------------------------------------------------------*/
/* USER CODE BEGIN EC */

/* USER CODE END EC */

/* Exported macro ------------------------------------------------------------*/
/* USER CODE BEGIN EM */

/* USER CODE END EM */

/* Exported functions prototypes ---------------------------------------------*/
void Error_Handler(void);

/* USER CODE BEGIN EFP */

/* USER CODE END EFP */

/* Private defines -----------------------------------------------------------*/
#define M1_PWM_UL_Pin GPIO_PIN_13
#define M1_PWM_UL_GPIO_Port GPIOC
#define CAN_TERMINATION_Pin GPIO_PIN_14
#define CAN_TERMINATION_GPIO_Port GPIOC
#define M1_BUS_VOLTAGE_Pin GPIO_PIN_0
#define M1_BUS_VOLTAGE_GPIO_Port GPIOA
#define SHUNT_U_Pin GPIO_PIN_2
#define SHUNT_U_GPIO_Port GPIOA
#define M1_OPAMP1_INT_GAIN_Pin GPIO_PIN_3
#define M1_OPAMP1_INT_GAIN_GPIO_Port GPIOA
#define BEMF_U_Pin GPIO_PIN_4
#define BEMF_U_GPIO_Port GPIOA
#define SHUNT_V_Pin GPIO_PIN_6
#define SHUNT_V_GPIO_Port GPIOA
#define BEMF_V_Pin GPIO_PIN_4
#define BEMF_V_GPIO_Port GPIOC
#define SHUNT_W_Pin GPIO_PIN_1
#define SHUNT_W_GPIO_Port GPIOB
#define BEMF_W_Pin GPIO_PIN_11
#define BEMF_W_GPIO_Port GPIOB
#define DEVBOARD_POT_Pin GPIO_PIN_12
#define DEVBOARD_POT_GPIO_Port GPIOB
#define M1_TEMPERATURE_Pin GPIO_PIN_14
#define M1_TEMPERATURE_GPIO_Port GPIOB
#define M1_PWM_WL_DISABLED_Pin GPIO_PIN_15
#define M1_PWM_WL_DISABLED_GPIO_Port GPIOB
#define DEVBOARD_LED_Pin GPIO_PIN_6
#define DEVBOARD_LED_GPIO_Port GPIOC
#define M1_PWM_UH_Pin GPIO_PIN_8
#define M1_PWM_UH_GPIO_Port GPIOA
#define M1_PWM_VH_Pin GPIO_PIN_9
#define M1_PWM_VH_GPIO_Port GPIOA
#define M1_PWM_WH_DISABLED_Pin GPIO_PIN_10
#define M1_PWM_WH_DISABLED_GPIO_Port GPIOA
#define M1_PWM_VL_Pin GPIO_PIN_12
#define M1_PWM_VL_GPIO_Port GPIOA
#define PWM_INPUT_Pin GPIO_PIN_15
#define PWM_INPUT_GPIO_Port GPIOA
#define DEVBOARD_BUTTON_Pin GPIO_PIN_10
#define DEVBOARD_BUTTON_GPIO_Port GPIOC
#define CAN_TRANSCEIVER_SHUTDOWN_Pin GPIO_PIN_11
#define CAN_TRANSCEIVER_SHUTDOWN_GPIO_Port GPIOC

/* USER CODE BEGIN Private defines */

/* USER CODE END Private defines */

#ifdef __cplusplus
}
#endif

#endif /* __MAIN_H */
